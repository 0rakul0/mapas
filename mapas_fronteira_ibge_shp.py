...
if __name__ == "__main__":
    m = mapa()
    shapefile_path = "./IBGE/Municipios_Faixa_Fronteira_2022.shp"
    gdf = gpd.read_file(shapefile_path, encoding='utf-8')
    print(gdf.info())
    print('tamanho', gdf.shape)

    gdf.plot(figsize=(20,15))
    plt.title("Mapa do intermediarios do Brasil")
    plt.axis('off')  # Remova os eixos
    plt.show()
    plt.savefig('fig_mapa/mapa_fronteira_com_outro_pais.png')
    ...
    df_vizinhanca.to_csv('data_master/vizinhos_municipios_fronteira_ibge.csv', index=False, quoting=1)